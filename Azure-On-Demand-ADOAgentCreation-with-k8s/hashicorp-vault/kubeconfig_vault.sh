...
vault kv get -format=yaml $deploy_environment/kubeconfig/$kube_context > secret.yaml
yq -r .data.config secret.yaml  > kubeconfigs/$kube_context

if [[ -n "$remote_context" ]]
then
    vault kv get -format=yaml $deploy_environment/kubeconfig/$remote_context > secret-remote.yaml
    yq -r .data.config secret-remote.yaml  > kubeconfigs/$remote_context    
fi