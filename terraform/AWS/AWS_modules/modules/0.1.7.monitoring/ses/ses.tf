locals {
  # some ses resources don't allow for the terminating '.' in the domain name # so use a replace function to strip it out
...
## DOMAIN IDENTITY #Description : Terraform module to create domain identity using domain
...
## DOMAIN IDENTITY VERIFICATION #Description : Terraform module to verify domain identity using domain
...
## DOMAIN IDENTITY VERIFICATION ROUTE53 #Description : Terraform module to record of Route53 for verify domain identity using domain
...
# # DOMAIN DKIM # Description : Terraform module which creates Domain DKIM resource on AWS
...
## DOMAIN DKIM VERIFICATION #Description : Terraform module to verify domain DKIM on AWS
...
## DOMAIN MAIL FROM #Description : Terraform module to create domain mail from on AWS
...
## SPF RECORD #Description : Terraform module to create record of SPF for domain mail from
...
## SPF RECORD #Description : Terraform module to create record of SPF for domain
...
## MX RECORD #Description : Terraform module to create record of MX for domain mail from
...
## MX RECORD #Description : Terraform module to create record of MX for receipt
...
## SES FILTER #Description : Terraform module to create receipt filter on AWS
...
## SES BUCKET POLICY #Description : Document of Policy to create Identity policy of SES
...
## SES IDENTITY POLICY #Description : Terraform module to create ses identity policy on AWS
...
## SES TEMPLATE #Description : Terraform module to create template on AWS
...
# # IAM USER # Description : Terraform module which creates SMTP Iam user resource on AWS
resource "aws_iam_user" "default" {
  count = var.iam_name != "" ? 1 : 0
  name  = var.iam_name
}

# # IAM ACCESS KEY # Description : Terraform module which creates SMTP Iam access key resource on AWS
resource "aws_iam_access_key" "default" {
  count = var.iam_name != "" ? 1 : 0
  user  = join("", aws_iam_user.default.*.name)
}

# # IAM USER POLICY # Description : Terraform module which creates SMTP Iam user policy resource on AWS
...
# # IAM USER POLICY DOCUMENT # Description : Terraform module which creates SMTP Iam user policy document resource on AWS
